...
def export_stl(sw_doc, path, output_filename):
    ...
    my_mesh.rotate([1.0, 0.0, 0.0,], math.radians(-90))
...
def export_file(sw_doc, path, filename, mode):
    """
    ...
    for sw_conf in sw_confs:
        ...
        if file_mode == "STL":
            export_stl(sw_doc, path, output_filename)
        elif file_mode == "DXF":
            export_dxf(sw_doc, path, output_filename, filename)
...
@click.argument(
    "mode",
    type=click.Choice(["Auto", "STL", "DXF"])
    )
def auto_export(input_path, mode) -> None:
    ...
    dxf_path = os.path.join(input_path, "DXF")
    stl_path = os.path.join(input_path, "STL")
    ...
    for root, _, files in os.walk(input_path):
        ...
        for file in files:
            ...
            export_file(sw_doc, root, file, mode)
